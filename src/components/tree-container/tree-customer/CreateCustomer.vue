<template>
	<w-dailog v-model="dialogVisible" :title="`${dialogTitle}客户类别`" width="460px">
...
export default {
	...
	methods: {
		...
		getArchiveCode () {
			return this.$fxApi('archiveCode.get', 'Customer_Type')
		},
		onSaveClick () {
		...
		onSaveHandler () {
			if (this.actionType === 'add') {
				const logger = this.$fxLogger.createInfo('basic-archives.customer-manage.customerTypeAdd', { name: this.formData.name })
				return this.$fxApi('customerType.add')({ data: this.formData, logger }).then(res => {
					return Promise.resolve()
				})
			} else {
				const logger = this.$fxLogger.createInfo('basic-archives.customer-manage.customerTypeUpdate', { name: this.formData.name })
				return this.$fxApi('customerType.save', this.formData.id)({ data: this.formData, logger }).then(res => {
					return Promise.resolve()
				})
			}
		},
