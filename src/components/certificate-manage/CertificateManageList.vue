<template>
	<div>
		...
		>
			<w-simple-table-column prop="typeName" label="证照类型" width="110px">
				<span slot-scope="scope">{{getResourceTypeName(scope.row.type)}}</span>
			</w-simple-table-column>
			<w-simple-table-column prop="name" label="证照名称" width="115px">
			...
			<w-simple-table-column prop="code" label="证照编号" width="120px">
			...
			<w-simple-table-column prop="date" label="证照有效期" width="170px" align="center">
				<span slot-scope="scope">{{scope.row.begin ? `${scope.row.begin}-${scope.row.end}` : ''}}</span>
			</w-simple-table-column>
			<w-simple-table-column prop="file" label="附件" width="70px" align="center">
...
export default {
	...
	methods: {
		...
		onCertificateEditClick ({ row, index }) {
			this.$refs.createDialog.editOpen(row)
		},
