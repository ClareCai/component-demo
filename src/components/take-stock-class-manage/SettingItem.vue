<template>
	<w-dailog v-model="dialogVisible" width="1200px" title="设置适用品项" heightFullscreen @close="onClose">
...
export default {
	...
	methods: {
		...
		onSaveClick () {
			const params = this.$refs.transfer.getSelectRows().map(item => {
				return {
					itemId: item.id,
			...
			return new Promise(resolve => {
				...
				this.$fxApi(`${apiName}.settingItem`, this.itemDetails.id)({ data: params, logger }).then(() => {
					this.fxDataVerification.resetData(selectRows)
					resolve()
				})
			})
