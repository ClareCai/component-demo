	...
	>
		...
		<w-scroll-bar class="scroll-container">
			<w-tree
				...
				emptyText="无上下级单据"
			>
				<div class="bill-container" slot-scope="{ data }" :class="{'current-bill':data.currentBill}" @click="onTreeNodeClick">
					...
					<div class="bill-content">
						...
						<div class="bill-state">
							<fx-detials-state-tag v-if="data.billState!==2" :label="currentState(data).label" :icon="currentState(data).icon"></fx-detials-state-tag>
							<fx-detials-state-tag :label="assignStatus(data).label" :icon="assignStatus(data).icon"></fx-detials-state-tag>
							<fx-detials-state-tag :label="inAssignStatus(data).label" :icon="inAssignStatus(data).icon"></fx-detials-state-tag>
							<fx-detials-state-tag :label="outAssignStatus(data).label" :icon="outAssignStatus(data).icon"></fx-detials-state-tag>
							<fx-detials-state-tag :label="currentMoveState(data).label" :icon="currentMoveState(data).icon"></fx-detials-state-tag>
							<fx-detials-state-tag :label="offsetState(data).label" :icon="offsetState(data).icon"></fx-detials-state-tag>
						</div>
					</div>
				</div>
			</w-tree>
