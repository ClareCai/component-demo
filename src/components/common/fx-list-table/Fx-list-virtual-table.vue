 ...
 * @param {Boolean} headerBorder - 开启表头边框
...
export default {
		...
		selectTotalComputed () {
			if (this.selectTotalGlobal) {
				...
				return this.selectedRowArr.filter(item => {
					return listDataCompareArrayUtils.has(item.id)
				}).length
			}
		},
		computedIndex () {
	...
	methods: {
				...
				if (!row) {
					...
					if (!selecteds.length) {
						...
						selecteds.forEach(item => {
							if (!this.compareArrayUtils.has(item.id)) {
		...
		onTableRowClick (item, column, index, event) {
			if (!this.selectable || !column) {
				return false
			}
			if (typeof this.selectableHandler === 'function' && !this.selectableHandler(item, index)) {
				return false
			}
		...
		cellTabHandler (cellEvent) {
			if (this.tableEditColumnProp.length > 0) {
				...
				if (typeIndex !== this.tableEditColumnProp.length - 1) {
					...
					const isExpandCell = this.$refs.organTable.isRowExpanded(cellEvent.row.id)
		...
		renderSelect () {
			...
			if (this.selectMulity) {
				this.listData.forEach(item => {
					this.$refs.organTable.onlyToggleRowSelection(item, !!this.compareArrayUtils.has(item.id))
				})
