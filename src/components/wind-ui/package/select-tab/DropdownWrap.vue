<template>
	<w-scroll-bar ref="scroll-bar" class="wind-select-tab__scroll-bar" wrapClass="wind-select-tab__dropdown-wrap">
		<div v-show="currentOptionList.length>0&&!loading" ref='phantom' :style="{height: `${this.phantomHeight}px`}" class="wind-select-tab__dropdown-list">
			<ul
				:style="{
...
export default {
	...
	computed: {
		selectItemClasses () {
			return (listItem) => {
				return {
					selected: this.selectValue === listItem.id,
	...
	watch: {
		optionlist: {
			...
			handler (value, oldValue) {
				this.currentOptionList = value.filter(item => item.visibleFlag !== false)
				this.$nextTick(() => {
