		...
		>
			<div class="search-input">
				<el-input
					...
					@keypress.enter.native="onSelectEnter"
					@blur="onInputBlur"
				>
			...
			<el-tabs v-show="currentOptionList.length>0&&!loading" ref="el-tab" v-model="activeTag">
				<template v-for="(item, index) in currentOptionList">
					<el-tab-pane :name="item.id" :key="index">
						<div slot="label">
							<fx-render-dom
								v-if="paneRender"
								:render="paneRender"
							...
							></fx-render-dom>
							<span v-else>{{item[optionProp.tabName]}}</span>
						</div>
						<dropdown-wrap
...
export default {
	...
	computed: {
		...
		tagMoreText () {
			if (this.value.length > 1) {
				...
				const tips = this.optionlist.reduce((pre, cur) => {
		...
		popperVisible (val) {
			if (val) {
				...
				this.optionlist.forEach(item => {
					this.$refs[`dropdown-wrap-${item.id}`][0].scrollToIndex(0)
				})
			} else {
				this.activeTag = ''
		...
		searchKey (val) {
			if (val) {
				this.searchDebounce(200, false).then(() => {
					if (this.currentTag.listData.length === 1) {
	...
	methods: {
		...
		onSelectAll () {
			const value = this.currentOptionList.reduce((pre, cur) => {
				pre.push(
					...cur.listData
						.filter(
							item => !item[this.optionProp.disabled || 'disabled'] && item[this.optionProp.visibleFlag || 'visibleFlag'] !== false
						)
						.map(item => item.id))
				return pre
			}, [])
