...
export default {
	...
	theoryCostOrder: {
		...
		getBillBizTypeSelect () {
			return post({ url: '/archive/bill/bill_biz_type/v1/bill_type/301', loading: false })
		},
		// 获取单据类型list数据
		// getBilltypeSelect () {
		...
		edit (id) {
			return put({
				...
				autoSuccessMsg: true,
				data: {
					...app.vue.$fxCommon.getBillUpdateVersion('theoryCost', id),
					...app.vue.$fxCommon.getTenantIdParam()
				},
				success: res => app.vue.$fxCommon.normalizationBillUpdateVersion('theoryCost', res)
			})
		},
		// 获取单据详情
		getBill (id) {
		...
		getDetails (billId) {
			return get({
				...
				loading: false,
				success: res => {
					const { details } = app.vue.$fxCommon.normalizationBillUpdateVersion('theoryCost', res)
					return details
				}
			})
		},
		...
		editDetail (billId, id) {
			return put({
				...
				data: { ...app.vue.$fxCommon.getBillDetailUpdateVersion('theoryCost', billId) },
				success: res => app.vue.$fxCommon.normalizationBillDetailUpdateVersion('theoryCost', res, billId)
			})
		},
		// 删除单据
		delete (id) {
		...
		setMulity (billId) {
			return put({
				...
				autoSuccessMsg: true,
				data: {
					...app.vue.$fxCommon.getBillDetailUpdateVersion('theoryCost', billId)
				}
			})
		},
		// 明细查询库存
