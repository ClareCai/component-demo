	...
	common: {
		...
		getSupportClassTree () {
			return get({
				...
				success: (res) => {
					const [financialClass, bussinessType, inventoryProfitReason, takeStockClass, invoiceType] = COMMON_TYPE_CONFIG.supportClass
					invoiceType.children = COMMON_TYPE_CONFIG.invoiceTypeList
					const children = res[0].children
					res[0].children = [financialClass, { ...bussinessType, children }, inventoryProfitReason, takeStockClass, invoiceType]
