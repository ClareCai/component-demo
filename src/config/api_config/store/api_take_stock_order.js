...
export default {
	...
	takeStockOrder: {
		...
		delDetailMulity (billId) {
			return put({ url: `/bill/check/${billId}/detail/v1/delete`, autoSuccessMsg: true })
		},
		// 审核
		check (id) {
		...
		settingMulity (billId) {
			return put({ url: `/bill/check/${billId}/detail/v1/set`, autoSuccessMsg: true })
		},
		// 导入必盘品项接口
		...
		getBillBizTypeSelect () {
			return post({ url: '/archive/bill/bill_biz_type/v1/bill_type/102', data: [] })
		},
		// 分页查询-单据类型下拉列表
		getBilltypeSelect () {
		...
		// 可盘品项列表
		getCheckItemList () {
			return post({ url: '/bill/check/v1/itemList' })
		},
		// 盘点机构列表
		getOrganTabList () {
		...
		// 提交前的防错校验接口
		checkMistake (billId) {
			return post({ url: `/bill/check/v1/mistake_proofing/${billId}` })
		},
		// 删除重复品项
