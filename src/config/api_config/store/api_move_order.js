...
export default {
	...
	moveOrder: {
		...
		// 反审店内调拨单
		uncheckIn (id) {
			return put({ url: `/bill/move_apply/v1/${id}/in/uncheck`, autoSuccessMsg: true })
		},
		// 批量反审
		uncheckMulity () {
			return put({ url: '/bill/move_apply/v1/uncheck', autoSuccessMsg: true })
		},
		// 批量审核
		checkMulity () {
		...
		// 批量同意
		agreeMulity () {
			return put({ url: '/bill/move_apply/v1/agree/batch', autoSuccessMsg: true })
		},
		// 批量拒绝
		rejectMulity () {
		...
		delDetail (billId, detailId) {
			return del({ url: `/bill/move_apply/${billId}/detail/v1/${detailId}`, autoSuccessMsg: true })
		},
		// 批量新增明细
		...
		delDetailMulity (billId) {
			return put({ url: `/bill/move_apply/${billId}/detail/v1/delete`, autoSuccessMsg: true })
		},
		// 根据单据id清空单据明细
		clearDetails (billId) {
		...
		// 反审店间调拨
		uncheckOut (id) {
			return put({ url: `/bill/move_apply/v1/${id}/out/uncheck`, autoSuccessMsg: true })
		},
		// 根据仓库查询品项接口
		getItemByStore (billId, houseId, itemId = 0) {
			return get({ url: `/bill/move_apply/${billId}/detail/v1/house/${houseId}/itemList/${itemId}` })
		},
		// 查询全部品项
		getItemAll (billId) {
			return get({ url: `/bill/move_apply/${billId}/detail/v1/itemList` })
		},
		// 根据品项获取单位列表
		getItemUnitList (billId, itemId) {
			return post({ url: `/bill/move_apply/${billId}/detail/v1/unitList/${itemId}` })
		},
		// 批量设置明细
		...
		clearDetailsOutstore (billId) {
			return put({ url: `/bill/move_apply/${billId}/detail/v1/batch_set/house`, data: { inOrOut: 0, houseId: '' } })
		},
		// 清空明细入库仓库
		clearDetailsInstore (billId) {
			return put({ url: `/bill/move_apply/${billId}/detail/v1/batch_set/house`, data: { inOrOut: 1, houseId: '' } })
		},
		// 明细查询库存
		...
		getBillBizTypeSelect () {
			return post({ url: '/archive/bill/bill_biz_type/v1/bill_type/103', data: [] })
		},
		// 分页查询-单据类型下拉列表
		getBilltypeSelect () {
		...
		// 提交前的防错校验接口
		checkMistake (billId) {
			return post({ url: `/bill/move_apply/v1/mistake_proofing/${billId}` })
		},
		// 删除重复品项
		...
		// 查询仓库未关联品项
		getRel (billId) {
			return post({ url: `/bill/move_apply/v1/mistake_proofing/${billId}`, query: { error_check: false }, data: ['Belong'] })
		},
		// 查询调拨数量为0的品项
