...
export default {
	...
	purchaseInvoice: {
		...
		// 主列表筛选条件：获取业务类型列表
		getInvoiceTypeList () {
			return get({ url: '/archive/invoice_type/v1/list' })
		},
		// 主列表筛选条件：获取业务类型列表
		getInvoiceType4List () {
		...
		// 分页查询接口
		getPage () {
			return post({ url: '/bill/purchase_invoice/v1/page' })
		},
		// 分页查询接口（回收站）
		getPageRecycling () {
			return post({ url: '/bill/purchase_invoice/v1/page', query: { del_flag: true } })
		},
		// 获取采购方列表
		getBuyerList () {
		...
		// 删除发票
		del (id) {
			return put({ url: `/bill/purchase_invoice/v1/${id}/delete`, autoSuccessMsg: true })
		},
		// 批量删除发票
		deleteMulity () {
			return put({ url: '/bill/purchase_invoice/v1/delete', autoSuccessMsg: true })
		},
		// 恢复发票
		restore (id) {
			return put({ url: `/bill/purchase_invoice/v1/${id}/restore`, autoSuccessMsg: true })
		},
		// 批量恢复发票
		restoreMulity () {
			return put({ url: '/bill/purchase_invoice/v1/restore', autoSuccessMsg: true })
		},
		// 回收站删除发票
		remove (id) {
			return del({ url: `/bill/purchase_invoice/v1/${id}/remove`, autoSuccessMsg: true })
		},
		// 回收站批量删除发票
		removeMulity () {
			return del({ url: '/bill/purchase_invoice/v1/remove', autoSuccessMsg: true })
		},
		// 获取经办人接口
		getAgentList () {
		...
		// 新增发票
		add () {
			return post({ url: '/bill/purchase_invoice/v1', autoSuccessMsg: true })
		},
		// 编辑保存发票
		save (id) {
			return put({ url: `/bill/purchase_invoice/v1/${id}`, autoSuccessMsg: true })
		},
		// 查询发票
		getDetails (id) {
			return get({ url: `/bill/purchase_invoice/v1/${id}` })
		},
		// 获取机构统一社会信用编码资质
		getResource () {
