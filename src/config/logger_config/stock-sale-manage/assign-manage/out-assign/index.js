export default {
	...
	actions: [{
		...
		messageHandler ({ data }) {
			return data.map(i => {
				if (i.stockWay === 1) {
					return `系统单号：${this.createCodeSpan(i, 'SalesOrder')}，品项名称：${i.collectionData.itemName}，单位：${i.collectionData.unitName}，订货方式：直发， 本次出库数量：${i.thisAmount}，卖方仓库：${i.outStore}`
				} else {
					return `系统单号：${this.createCodeSpan(i, 'SalesOrder')}，品项名称：${i.collectionData.itemName}，单位：${i.collectionData.unitName}，订货方式：越库， 本次出库数量：${i.thisAmount}，卖方仓库：${i.outStore}，实际供货方：${i.crossOrganName || '/'}`
				}
			}).join('；')
		}
	}, {
